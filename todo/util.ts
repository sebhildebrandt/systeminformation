...
function unique(obj: any[]) {
  ...
  for (let i = 0; i < obj.length; i++) {
    ...
    keys.sort(function (a: string, b: string) { return (a < b ? -1 : (a > b ? 1 : 0)); });
    let str = '';
...
function sortByKey(array: any[], keys: string[]) {
...
function getValue(lines: string[], property: string, separator = ':', trimmed = false, lineMatch = false) {
...
function decodeEscapeSequence(str: string, base = 16) {
...
function parseTime(t: string, pmDesignator = '') {
  t = t.toUpperCase();
  let hour = 0;
  let min = 0;
...
function parseDateTime(dt: string, culture: any = {}) {
  const result = {
    date: '',
    time: ''
  };
...
function parseHead(head: string, rights: number) {
  let space = (rights > 0);
  let count = 1;
  let from = 0;
  let to = 0;
...
function getWmic() {
  if (os.type() === 'Windows_NT' && !_wmicPath) {
    _wmicPath = WINDIR + '\\system32\\wbem\\wmic.exe';
    if (!fs.existsSync(_wmicPath)) {
      try {
        const wmicPathArray = execSync('WHERE WMIC', execOptsWin).toString().split('\r\n');
        if (wmicPathArray && wmicPathArray.length) {
          _wmicPath = wmicPathArray[0];
        } else {
          _wmicPath = 'wmic';
        }
      } catch (e) {
...
function powerShell(cmd: string) {
...
function execSafe(cmd: string, args: any, options: any) {
  let result = '';
  options = options || {};
...
function execWin(cmd: string, opts: any, callback: any) {
  if (!callback) {
    callback = opts;
    opts = execOptsWin;
  }
  let newCmd = 'chcp 65001 > nul && cmd /C ' + cmd + ' && chcp ' + _codepage + ' > nul';
...
function countUniqueLines(lines: string[], startingWith = '') {
...
function countLines(lines: string[], startingWith = '') {
...
function sanitizeShellString(str: string, strict: any) {
...
function getFilesInPath(source: string) {
  ...
  function isFile(source: string) { return lstatSync(source).isFile(); }
...
function decodePiCpuinfo(lines: string[]) {
...
function promisify(nodeStyleFunction: any) {
  return function () {
...
function promisifySave(nodeStyleFunction: any) {
  return function () {
    var args = Array.prototype.slice.call(arguments);
    return new Promise(function (resolve) {
      args.push(function (err: any, data: any) {
...
function plistParser(xmlStr: string) {
  ...
  function getNextClosingTagPos(tag: string) {
    return xmlStr.indexOf(`</${tag}>`);
  }

  function parseXmlTree(isArray: boolean, closingTag: string): any {
    // start parsing
    let obj: any = {};
    let arr = [];
