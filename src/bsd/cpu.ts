...
export const cpu = async () => {
  ...
  const { stdout } = await execSave('dmidecode -t 4');
  if (stdout) {
    let cache = [];
    ...
    result.speedMax = Math.round(parseFloat(getValue(lines, 'max speed').replace(/Mhz/g, '')) / 10.0) / 100 || result.speed;
      ...
      if (cacheType) {
        switch (true) {
          case cacheType === 'l1':
            ...
            break;
          case cacheType === 'l2':
            result.cache.l2 = size;
            break;
          case cacheType === 'l3':
            result.cache.l3 = size;
        }
      }
    }
    // socket type
    ...
    const brandManufacurer = cpuBrandManufacturer(cpus()[0].model);
    return {
      ...result,
      speed: Number((cpus()[0].speed / 1000).toFixed(2)),
      brand: brandManufacurer.brand,
      manufacturer: brandManufacurer.manufacturer
    };
  }
  return result;
};
