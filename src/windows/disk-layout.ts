...
const hasControlCharacters = (str: string) => /\p{Cc}/u.test(str);

const diskSerialNum = (device: any): string => {
  const serialNum = String(device.SerialNumber || '').trim();
  if (!hasControlCharacters(serialNum)) {
    return serialNum;
  }
  const instanceId = String(device.PNPDeviceId || '')
    .trim()
    .split('\\')[2];
  if (instanceId) {
    ...
    psArray(diskDrives).forEach((device: any) => {
      ...
      if (size) {
        result.push({
          ...
          serialNum: diskSerialNum(device),
