...
- `networkStats()` added `rx_packets` and `tx_packets` (received / transferred packets overall, #337)
...
- `displays()` added `gpu` and `gpuBusAddress` to link a display to the graphics controller driving it (#974). The platforms expose opposite halves of this: Linux knows the PCI address of the connector's card (`/sys/class/drm/cardN/device`, same value as `gpu()[].busAddress`) but not the model name, macOS knows the model because `system_profiler` nests the displays under their GPU but has no bus address, Windows resolves the adapter per display via `EnumDisplayDevices` and its PCI address from the PnP location info, so two identical cards stay distinguishable there too. Unknown values stay `''`
- `gpu()` added `busAddress` on Windows (PCI address in the same `01:00.0` notation as on Linux, read from `DEVPKEY_Device_LocationInfo`). On Linux it was already returned but undocumented
...
- `blockDevices()` and `fsSize()` report the usable instead of the raw capacity for btrfs. The pool entry summed its member sizes and btrfs reports the raw device capacity as filesystem size, so a raid5 of four disks showed the full four disks (#883). The pool size is now derived from the data profile (raid5 minus one device, raid6 minus two, raid1/raid10/dup halved, raid1c3/raid1c4 divided by three/four), and `fsSize()` uses `used + available`, which already accounts for the profile - the same values `use` has always been computed from. Note that the reported size therefore **shrinks** on mirrored profiles compared to earlier betas
...
- `mem()` `active` and `available` now account for the ZFS ARC cache - the ARC is not part of the page cache, so `MemAvailable` (Linux) and the page counters (FreeBSD) report it as used. On a ZFS host `active` therefore sat near `total` permanently and real memory growth was invisible. Everything the ARC holds above `c_min` is reclaimable and is now counted as available, the same correction htop applies (Linux/FreeBSD, #808)
...
- `cpuTemperature()` matches the CPU thermal zone case insensitively - Tegra / Jetson names it `CPU-therm`, so the main temperature stayed empty there (Linux)
- `gpu()` takes `vram` for discrete cards from sysfs instead of the PCI region size. The largest `Region N ... [size=]` of `lspci` was only overwritten when no region had been found at all, so the sysfs value never won - a Radeon 7900 XTX reported `vram: 32768` because resizable BAR exposes a 32 GB window on a 24 GB card (#1035). `mem_info_vram_total` (amdgpu) now wins, and discrete Intel Arc is read as well through `lmem_total_bytes` / `tile0/physical_vram_size_bytes`. NVIDIA is unaffected, its value keeps coming from `nvidia-smi` (Linux)
- `gpu()` no longer reports the PCI graphics aperture as `vram` for integrated GPUs. The largest `Region N ... [size=]` of `lspci` is the CPU visible aperture, not video memory - an Intel UHD Graphics 630 was reported as `vram: 256, vramDynamic: false` although it has no dedicated memory at all and takes system RAM on demand. Cards whose DRM driver is an integrated one (`i915`, `xe`, `v3d`, `vc4`, `panfrost`, `lima`, `msm`, `etnaviv`) and that expose no VRAM total node in sysfs now return `vram: null` with `vramDynamic: true`. Discrete cards are unaffected: AMD has `mem_info_vram_total`, discrete Intel Arc has `lmem_total_bytes` / `physical_vram_size_bytes`, and NVIDIA keeps the value from `nvidia-smi` (Linux)
