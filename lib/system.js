        ...
        exec(
          ...
          function (error, stdout) {
            ...
            try {
              ...
              result.model =
                result.model === ''
                  ? util.getValue(lines, 'product_name')
                  : result.model;
              result.version =
                result.version === ''
                  ? util.getValue(lines, 'product_version')
                  : result.version;
              result.serial =
                result.serial === ''
                  ? util.getValue(lines, 'product_serial')
                  : result.serial;
              result.uuid =
                result.uuid === ''
            ...
            if (
              !result.virtual &&
              (os.release().toLowerCase().indexOf('microsoft') >= 0 ||
            ...
            if ((_freebsd || _openbsd || _netbsd) && !result.virtualHost) {
              try {
                ...
                const procManufacturer = util.getValue(
                  procLines,
                  'manufacturer',
                  ':',
                  true
      ...
      if (_darwin) {
        exec('ioreg -c IOPlatformExpertDevice -d 2', function (error, stdout) {
          if (!error) {
            ...
            result.uuid = util
              .getValue(lines, 'ioplatformuuid', '=', true)
              .toLowerCase();
    ...
    process.nextTick(() => {
      ...
      if (_linux || _freebsd || _openbsd || _netbsd) {
        ...
        exec(cmd, function (error, stdout) {
          ...
          let language = util
            .getValue(lines, 'Currently Installed Language')
            .split('|')[0];
          if (language) {
          ...
          try {
            ...
            result.vendor = !result.vendor
              ? util.getValue(lines, 'bios_vendor')
              : result.vendor;
            ...
            result.releaseDate = !result.releaseDate
              ? util.parseDateTime(datetime).date
              : result.releaseDate;
          } catch (e) {
            util.noop();
          }
        ...
        util.promiseAll(workload).then((data) => {
          ...
          try {
            ...
            result.manufacturer = !result.manufacturer
              ? util.getValue(lines, 'board_vendor')
              : result.manufacturer;
            result.model = !result.model
              ? util.getValue(lines, 'board_name')
              : result.model;
            result.version = !result.version
              ? util.getValue(lines, 'board_version')
              : result.version;
            result.serial = !result.serial
              ? util.getValue(lines, 'board_serial')
              : result.serial;
            result.assetTag = !result.assetTag
              ? util.getValue(lines, 'board_asset_tag')
              : result.assetTag;
          } catch (e) {
            util.noop();
          }
          ...
          lines = data.results[1]
            ...
            : [''];
          result.memMax =
            util.toInt(util.getValue(lines, 'Maximum Capacity')) *
              1024 *
              1024 *
              1024 || null;
          result.memSlots =
            util.toInt(util.getValue(lines, 'Number Of Devices')) || null;
          // raspberry
          let linesRpi = '';
          try {
        ...
        try {
          ...
          util.promiseAll(workload).then((data) => {
            ...
            lines = data.results[1]
              ...
              : [''];
            result.memMax =
              util.toInt(util.getValue(lines, 'MaxCapacity', ':')) || null;
            result.memSlots =
              util.toInt(util.getValue(lines, 'MemoryDevices', ':')) || null;
            if (callback) {
              callback(result);
            }
            resolve(result);
      ...
      if (_linux || _freebsd || _openbsd || _netbsd) {
        ...
        exec(cmd, function (error, stdout) {
          ...
          const ctype = parseInt(
            util.getValue(lines, 'chassis_type').replace(/\D/g, '')
          );
          result.type =
          ...
          util
            ...
            .then((stdout, error) => {
              if (!error) {
                ...
                result.model = util.getValue(lines, 'model', ':');
                const ctype = parseInt(
                  util.getValue(lines, 'ChassisTypes', ':').replace(/\D/g, '')
                );
                result.type =
                  ctype && !isNaN(ctype) && ctype < chassisTypes.length
                    ? chassisTypes[ctype - 1]
