...
function graphics(callback) {
  ...
  function parseLinesLinuxControllers(lines) {
    ...
    let pciIDs = [];
    try {
      pciIDs = execSync('export LC_ALL=C; dmidecode -t 9 2>/dev/null; unset LC_ALL | grep "Bus Address: "', { stdio: ['pipe', 'pipe', 'ignore'] }).toString().split('\n');
      ...
      if (_darwin) {
        ...
        exec(cmd, function (error, stdout) {
          if (!error) {
            ...
            try {
              stdout = execSync('defaults read /Library/Preferences/com.apple.windowserver.plist 2>/dev/null;defaults read /Library/Preferences/com.apple.windowserver.displays.plist 2>/dev/null; echo ""', { maxBuffer: 1024 * 20000, stdio: ['pipe', 'pipe', 'ignore'] });
              const output = (stdout || '').toString();
