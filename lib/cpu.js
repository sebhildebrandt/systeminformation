    ...
    process.nextTick(() => {
      ...
      if (now >= 200) {
        ...
        if (options && options.constructor === Object) {
          rawData = options.raw === true ? true : false;
        }
        ...
        for (let i = 0; i < _corecount; i++) {
          ...
          let loadData = (_cpus[i].totalLoad - tmp_load);
          let loadUser = (_cpus[i].user - tmp_user);
          let loadSystem = (_cpus[i].sys - tmp_system);
          let loadNice = (_cpus[i].nice - tmp_nice);
          let loadIdle = (_cpus[i].idle - tmp_idle);
          let loadIrq = (_cpus[i].irq - tmp_irq);

          if (!rawData) {
            loadData = loadData / _cpus[i].currentTick * 100;
            loadUser = loadUser / _cpus[i].currentTick * 100;
            loadSystem = loadSystem / _cpus[i].currentTick * 100;
            loadNice = loadNice / _cpus[i].currentTick * 100;
            loadIdle = loadIdle / _cpus[i].currentTick * 100;
            loadIrq = loadIrq / _cpus[i].currentTick * 100;
          }

          _cpus[i].load = loadData;
          _cpus[i].load_user = loadUser
          _cpus[i].load_system = loadSystem
          _cpus[i].load_nice =  loadNice;
          _cpus[i].load_idle =  loadIdle;
          _cpus[i].load_irq = loadIrq;
          cores[i] = {};
        ...
        result = {
          avgload: avgload,
          currentload: (totalLoad - _current_cpu.load),
          currentload_user: (totalUser - _current_cpu.user),
          currentload_system: (totalSystem - _current_cpu.system),
          currentload_nice: (totalNice - _current_cpu.nice),
          currentload_irq: (totalIrq - _current_cpu.irq),
          cpus: cores
        };

        if (!rawData) {
          result.currentload = result.currentload / currentTick * 100;
          result.currentload_user = result.currentload / currentTick * 100;
          result.currentload_system = result.currentload / currentTick * 100;
          result.currentload_nice = result.currentload / currentTick * 100;
          result.currentload_irq = result.currentload / currentTick * 100;
        }
...
function currentLoad(options, callback) {

  return new Promise((resolve) => {
    process.nextTick(() => {
      getLoad(options).then(result => {
        if (callback) { callback(result) }
        resolve(result);
      })
    });
