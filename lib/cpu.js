    ...
    process.nextTick(() => {
      ...
      if (_freebsd) {
        exec('export LC_ALL=C; dmidecode ; unset LC_ALL', function(error, stdout) {
          if (!error) {
            console.log(stdout);
            var data = util.dmidecodeToObject(stdout)
            var cpu = data["Processor Information"];
            var cache = data["Cache Information"];
            console.log(cpu);
            result.brand = cpu.family;
            result.vendor = cpu.Manufacturer;
            const modelline = cpu.Version;
            ...
            _cpu_speed = result.speed;
            result.speedmax = Math.round(parseFloat(cpu['Max Speed'].replace(/Mhz/g, '')) / 10.0) / 100;
            result.speedmax = result.speedmax ? parseFloat(result.speedmax).toFixed(2) : '';

            result = cpuBrandManufacturer(result);
            result.vendor = cpu.Manufacturer;
            var sig = cpu.Signature;
            sig = sig.split(',');
            // needed to clean up whitespace
            ...
            for (var i = 0; i < cache.length; i++) {
              var cachetype = cache[i]['Socket Designation'].split(' ')[0].toLowerCase();
              console.log(cachetype);
              var size = cache[i]['Installed Size'].split(' ')[0];
              var unit = cache[i]['Installed Size'].split(' ')[1].toLowerCase();
              if (unit == 'kb') {
                size *= 1024;
              }
              if (unit == 'mb') {
      ...
      if (_windows) {
        exec(util.getWmic() + ' cpu get l2cachesize, l3cachesize /value', function(error, stdout) {
          if (!error) {
            ...
            if (result.l2) {
              result.l2 = parseInt(result.l2) * 1024;
            }
            if (result.l3) {
              result.l3 = parseInt(result.l3) * 1024;
            }
          }
          exec(util.getWmic() + ' path Win32_CacheMemory get CacheType,InstalledSize,Purpose', function(error, stdout) {
            if (!error) {
