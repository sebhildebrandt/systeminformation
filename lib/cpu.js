    ...
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        try {
          ...
          const parts = execSync(cmd, { stdio: ['pipe', 'pipe', 'ignore'] }).toString().split('-----\n');
          if (parts.length === 2) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (now >= 200) {
        ...
        if (_linux) {
          try {
            const lines = execSync('cat /proc/stat 2>/dev/null | grep cpu', { encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] }).toString().split('\n');
            if (lines.length > 1) {
              lines.shift();
