...
function getCpu() {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        exec("cat /proc/cpuinfo | grep -E 'model name|Revision'", function (error, stdout) {
          if (!error) {
            let lines = stdout.toString().split('\n');
            let modelLine = lines[0].split(':')[1];
            ...
            _cpu_speed = result.speed;
            let revisionLine = lines[1].split(':')[1];
            result.revision = revisionLine.trim();
          }
          result = cpuBrandManufacturer(result);
          resolve(result);
        })
