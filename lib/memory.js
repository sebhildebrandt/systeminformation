      ...
      if (_linux || _freebsd || _openbsd || _netbsd) {
        exec('export LC_ALL=C; dmidecode -t memory 2>/dev/null | grep -iE "Size:|Type|Speed|Manufacturer|Form Factor|Locator|Memory Device|Serial Number|Voltage|Part Number"; unset LC_ALL', function (error, stdout) {
          ...
          if (!result.length) {
            ...
            // Try Raspberry PI
            try {
              let stdout = execSync('cat /proc/cpuinfo 2>/dev/null', { stdio: ['pipe', 'pipe', 'ignore'] });
              let lines = stdout.toString().split('\n');
              ...
              if (model === 'BCM2835' || model === 'BCM2708' || model === 'BCM2709' || model === 'BCM2835' || model === 'BCM2837') {
                ...
                stdout = execSync('vcgencmd get_config sdram_freq 2>/dev/null', { stdio: ['pipe', 'pipe', 'ignore'] });
                ...
                stdout = execSync('vcgencmd measure_volts sdram_p 2>/dev/null', { stdio: ['pipe', 'pipe', 'ignore'] });
                lines = stdout.toString().split('\n');
