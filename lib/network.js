...
function getDefaultNetworkInterface() {
  ...
  try {
    ...
    if (_linux) {
      ...
      let result = execSync(cmd, { stdio: ['pipe', 'pipe', 'ignore'] });
...
function checkLinuxDCHPInterfaces(file) {
  ...
  try {
    ...
    const lines = execSync(cmd, { maxBuffer: 1024 * 20000, stdio: ['pipe', 'pipe', 'ignore'] }).toString().split('\n');
...
function getLinuxIfaceDHCPstatus(iface, connectionName, DHCPNics) {
  ...
  if (connectionName) {
    ...
    try {
      const lines = execSync(cmd, { stdio: ['pipe', 'pipe', 'ignore'] }).toString();
        ...
        if ((JSON.stringify(ifaces) === JSON.stringify(_ifaces)) && !rescan) {
          ...
          for (let dev in ifaces) {
            ...
            echo -n "wirelessspeed: "; iw dev ${ifaceSanitized} link 2>&1 | grep bitrate; echo;`;
              ...
              let lines = [];
              try {
                lines = execSync(cmd, { stdio: ['pipe', 'pipe', 'ignore'] }).toString().split('\n');
