...
function getDefaultNetworkInterface() {
  ...
  try {
    ...
    if (_windows) {
      ...
      let defaultIp = '';
      const cmd = 'netstat -r';
      const result = spawnSync(cmd, util.execOptsWin);
...
function getMacAddresses() {
  ...
  if (_linux || _freebsd || _openbsd || _netbsd) {
    if (typeof pathToIp === 'undefined') {
      try {
        const lines = spawnSync('which ip').toString().split('\n');
...
function getWindowsDNSsuffixes() {
  ...
  try {
    const ipconfig = spawnSync('ipconfig /all', util.execOptsWin);
...
function getWindowsWiredProfilesInformation() {
  try {
    const result = spawnSync('netsh lan show profiles', util.execOptsWin);
...
function getWindowsWirelessIfaceSSID(interfaceName) {
  try {
    const result = spawnSync(`netsh wlan show  interface name="${interfaceName}" | findstr "SSID"`, util.execOptsWin);
...
function getWindowsIEEE8021x(connectionType, iface, ifaces) {
  ...
  if (connectionType == 'wired' && ifaces.length > 0) {
    ...
    try {
      ...
      if (SSID !== 'Unknown') {
        i8021xState = spawnSync(`netsh wlan show profiles "${SSID}" | findstr "802.1X"`, util.execOptsWin);
        i8021xProtocol = spawnSync(`netsh wlan show profiles "${SSID}" | findstr "EAP"`, util.execOptsWin);
      }
...
function getDarwinNics() {
  ...
  try {
    const lines = spawnSync(cmd, { maxBuffer: 1024 * 20000 }).toString().split('\n');
...
function getLinuxIfaceConnectionName(interfaceName) {
  ...
  try {
    const result = spawnSync(cmd).toString();
...
function getLinuxIfaceDHCPstatus(iface, connectionName, DHCPNics) {
  ...
  if (connectionName) {
    ...
    try {
      const lines = spawnSync(cmd).toString();
...
function getDarwinIfaceDHCPstatus(iface) {
  ...
  try {
    const lines = spawnSync(cmd).toString().split('\n');
