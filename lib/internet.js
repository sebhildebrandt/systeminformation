  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux || _freebsd || _openbsd || _netbsd || _darwin) {
        ...
        util.execSafe('ping', params.split(' ')).then((stdout) => {
          let result = null;
          if (stdout) {
      ...
      if (_sunos) {
        ...
        const filt = 'avg';
        util.execSafe('ping', params.split(' '), { timeout: 3000 }).then((stdout) => {
          let result = null;
          if (stdout) {
            const lines = stdout.split('\n').filter(line => line.indexOf(filt) >= 0).join('\n');
            const line = lines.split('=');
      ...
      if (_windows) {
        ...
        try {
          ...
          util.execSafe('ping', params.split(' '), util.execOptsWin).then((stdout) => {
            if (stdout) {
