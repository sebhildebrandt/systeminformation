...
function ifaceListLinux() {
  ...
  try {
    const all = spawnSync(cmd).toString().split('\n').map(line => line.trim()).join('\n');
    ...
    try {
      const all = spawnSync('nmcli -t -f general,wifi-properties,wired-properties,interface-flags,capabilities,nsp device show 2>/dev/null').toString();
      const parts = all.split('\n\n');
      let i = 1;
      parts.forEach(ifaceDetails => {
        const lines = ifaceDetails.split('\n');
...
function getWifiNetworkListNmi() {
  ...
  try {
    const stdout = spawnSync(cmd, { maxBuffer: 1024 * 20000 });
...
function getWifiNetworkListIw(iface) {
  const result = [];
  try {
    let iwlistParts = spawnSync(`export LC_ALL=C; iwlist ${iface} scan 2>&1; unset LC_ALL`).toString().split('        Cell ');
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        if (result.length === 0) {
          try {
            const iwconfigParts = spawnSync('export LC_ALL=C; iwconfig 2>/dev/null; unset LC_ALL').toString().split('\n\n');
            let iface = '';
