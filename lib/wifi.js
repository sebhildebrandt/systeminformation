...
function ifaceListLinux() {
  ...
  try {
    const all = execSync(cmd, { stdio: ['pipe', 'pipe', 'ignore'] }).toString().split('\n').map(line => line.trim()).join('\n');
    ...
    try {
      const all = execSync('nmcli -t -f general,wifi-properties,wired-properties,interface-flags,capabilities,nsp device show 2>/dev/null', { stdio: ['pipe', 'pipe', 'ignore'] }).toString();
      const parts = all.split('\n\n');
      let i = 1;
      parts.forEach(ifaceDetails => {
        const lines = ifaceDetails.split('\n');
...
function nmiDeviceLinux(iface) {
  ...
  try {
    const lines = execSync(cmd, { stdio: ['pipe', 'pipe', 'ignore'] }).toString().split('\n');
...
function getWifiNetworkListNmi() {
  ...
  try {
    const stdout = execSync(cmd, { maxBuffer: 1024 * 20000, stdio: ['pipe', 'pipe', 'ignore'] });
...
function getWifiNetworkListIw(iface) {
  const result = [];
  try {
    let iwlistParts = execSync(`export LC_ALL=C; iwlist ${iface} scan 2>&1; unset LC_ALL`, { stdio: ['pipe', 'pipe', 'ignore'] }).toString().split('        Cell ');
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        if (result.length === 0) {
          try {
            const iwconfigParts = execSync('export LC_ALL=C; iwconfig 2>/dev/null; unset LC_ALL', { stdio: ['pipe', 'pipe', 'ignore'] }).toString().split('\n\n');
            let iface = '';
