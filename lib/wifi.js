...
function wifiPasswordWindows(ssid) {
  if (!ssid) {
    return Promise.resolve(null);
  }
  let ssidSanitized = '';
  const s = util.isPrototypePolluted() ? '---' : util.sanitizeShellString(ssid, true);
  const l = util.mathMin(s.length, 2000);
  for (let i = 0; i <= l; i++) {
    if (s[i] !== undefined) {
      ssidSanitized = ssidSanitized + s[i];
    }
  }
  if (!ssidSanitized) {
    return Promise.resolve(null);
  }
  const cmd = `netsh wlan show profile name="${ssidSanitized}" key=clear`;
  return util
    .powerShell(cmd)
    .then((stdout) => {
      ...
      const password = util.getValue(lines, 'Key Content', ':', true);
    ...
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        util.powerShell(cmd).then((stdout) => {
          ...
          const passwordTasks = result.map((item) =>
            wifiPasswordWindows(item.ssid).then((password) => {
              if (password !== null) {
