...
function getWmic() {
  if (os.type() === 'Windows_NT' && !wmicPath) {
    ...
    if (!fs.existsSync(wmicPath)) {
      try {
        const wmicPathArray = spawnSync('WHERE WMIC', execOptsWin).toString().split('\r\n');
...
function getCodepage() {
  if (_windows) {
    if (!codepage) {
      try {
        const stdout = spawnSync('chcp', execOptsWin);
  ...
  if (_linux || _darwin || _freebsd || _openbsd || _netbsd) {
    if (!codepage) {
      try {
        const stdout = spawnSync('echo $LANG');
        const lines = stdout.toString().split('\r\n');
        const parts = lines[0].split('.');
...
function smartMonToolsInstalled() {
  ...
  if (_windows) {
    try {
      const pathArray = spawnSync('WHERE smartctl 2>nul', execOptsWin).toString().split('\r\n');
  ...
  if (_linux || _darwin || _freebsd || _openbsd || _netbsd) {
    try {
      const pathArray = spawnSync('which smartctl 2>/dev/null', execOptsWin).toString().split('\r\n');
...
function linuxVersion() {
  ...
  if (_linux) {
    try {
      result = spawnSync('uname -v').toString();
    } catch (e) {
      result = '';
    }
  }
