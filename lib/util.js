...
async function getPowershell() {
  if (!_powerShell) {
    ...
    if (_windows) {
      ...
      if (await fs.promises.exists(defaultPath)) {
...
function wmic(command) {
  return new Promise((resolve) => {
    process.nextTick(async () => {
      try {
        powerShell((await getWmic()) + ' ' + command).then(stdout => {
          resolve(stdout, '');
        });
      } catch (e) {
        resolve('', e);
...
async function powerShellStart() {
  if (!_psChild) {
    _psChild = spawn(await getPowershell(), ['-NoProfile', '-NoLogo', '-InputFormat', 'Text', '-NoExit', '-Command', '-'], {
      stdio: 'pipe',
      windowsHide: true,
      maxBuffer: 1024 * 20000,
      encoding: 'UTF-8',
...
async function getCodepage() {
  if (_windows) {
    if (!codepage) {
      try {
        const stdout = await execAsync('chcp', execOptsWin);
  ...
  if (_linux || _darwin || _freebsd || _openbsd || _netbsd) {
    if (!codepage) {
      try {
        const stdout = await execAsync('echo $LANG', execOptsLinux);
        const lines = stdout.toString().split('\r\n');
        const parts = lines[0].split('.');
...
async function smartMonToolsInstalled() {
...
async function isRaspberry(cpuinfo) {
  const PI_MODEL_NO = [
    'BCM2708',
    'BCM2709',
    'BCM2710',
...
async function execWin(cmd, opts, callback) {
  if (!callback) {
    callback = opts;
    opts = execOptsWin;
  }
...
async function getRpiGpu(cpuinfo) {
...
async function linuxVersion() {
  let result = '';
  if (_linux) {
    try {
      result = (await execAsync('uname -v', execOptsLinux)).toString();
    } catch (e) {
      result = '';
    }
  }
