...
const codeset = process.platform === 'win32' ? getValue(execSync('wmic os get /value').toString().split('\r\n'), 'CodeSet', '=').toString().trim() : '';
...
function wmic(command, options) {
  options = Object.assign({}, options || execOptsWin);
  options.encoding = 'binrary';
  
  return new Promise((resolve) => {
    process.nextTick(() => {
      try {
        exec(getWmic() + ' ' + command, options, function (error, stdout) {
          const result = iconv.decode(Buffer.from(stdout, 'binary'), 'cp' + codeset).toString('UTF-8');
