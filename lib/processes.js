      ...
      if (srv) {
        ...
        if (_linux || _freebsd || _openbsd || _netbsd || _darwin) {
          if ((_linux || _freebsd || _openbsd || _netbsd) && srvString === '*') {
            try {
              const tmpsrv = execSync('systemctl --all --type=service --no-legend 2> /dev/null', { stdio: ['pipe', 'pipe', 'ignore'] }).toString().split('\n');
              srvs = [];
              ...
              try {
                srvString = '';
                const tmpsrv = execSync('service --status-all 2> /dev/null', { stdio: ['pipe', 'pipe', 'ignore'] }).toString().split('\n');
                for (const s of tmpsrv) {
                  const parts = s.split(']');
                  if (parts.length === 2) {
                ...
                try {
                  const srvStr = execSync('ls /etc/init.d/ -m 2> /dev/null', { stdio: ['pipe', 'pipe', 'ignore'] }).toString().split('\n').join('');
                  srvString = '';
                  if (srvStr) {
