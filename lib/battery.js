...
module.exports = (callback) =>
  new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_darwin) {
        exec(
          'ioreg -n AppleSmartBattery -r | egrep "BatteryData|NominalChargeCapacity|CycleCount|IsCharging|DesignCapacity|MaxCapacity|CurrentCapacity|DeviceName|BatterySerialNumber|Serial|TimeRemaining|Voltage"; pmset -g batt | grep %',
          (error, stdout) => {
            if (stdout) {
              let lines = stdout.toString().replace(/^[ |]+/gm, '').replace(/ +/g, '').replace(/"+/g, '').replace(/-/g, '').split('\n');
              // macOS 27 stores absolute capacities in BatteryData; MaxCapacity and CurrentCapacity are percentages.
              const batteryData = util.getValue(lines, 'BatteryData', '=').replace(/^\{/, '').replace(/\}$/, '').split(',');
              const maxCapacity = util.getValue(lines, 'AppleRawMaxCapacity', '=') || util.getValue(lines, 'NominalChargeCapacity', '=') || util.getValue(batteryData, 'FullChargeCapacity', '=') || util.getValue(batteryData, 'NominalChargeCapacity', '=');
              const currentCapacity = util.getValue(lines, 'AppleRawCurrentCapacity', '=') || util.getValue(batteryData, 'RemainingCapacity', '=');
              const designedCapacity = util.getValue(lines, 'DesignCapacity', '=') || util.getValue(batteryData, 'DesignCapacity', '=');
              result.cycleCount = parseInt('0' + util.getValue(lines, 'cyclecount', '='), 10);
              result.voltage = parseInt('0' + util.getValue(lines, 'voltage', '='), 10) / 1000.0;
              result.capacityUnit = result.voltage ? 'mWh' : 'mAh';
              result.maxCapacity = Math.round(parseInt('0' + maxCapacity, 10) * (result.voltage || 1));
              result.currentCapacity = Math.round(parseInt('0' + currentCapacity, 10) * (result.voltage || 1));
              result.designedCapacity = Math.round(parseInt('0' + designedCapacity, 10) * (result.voltage || 1));
              ...
              if (result.maxCapacity > 0 && currentCapacity !== '' && result.currentCapacity >= 0) {
                result.hasBattery = true;
                result.type = 'Li-ion';
