...
function systemLanguage(callback) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      let result = '';

      if (_windows) {
        const script = 'Get-WinUserLanguageList | ConvertTo-Json';
        exec(`powershell -Command "${script}"`, util.execOptsWin, (error, stdout) => {
          if (!error && stdout) {
            try {
              const languages = JSON.parse(stdout.toString());
              const available = Array.isArray(languages)
                ? languages.map((lang) => lang.LanguageTag)
      ...
      if (_darwin) {
        exec('defaults read NSGlobalDomain AppleLanguages', util.execOptsLinux, (error, stdout) => {
          if (!error && stdout) {
            const matches = stdout.toString().match(/"([a-zA-Z-]+)"/g);
            const languages = matches ? matches.map((value) => value.replace(/"/g, '')) : [];
            result = languages[0] || '';
          }
          if (callback) {
            callback(result);
      ...
      exec('locale', util.execOptsLinux, (error, stdout) => {
        if (!error && stdout) {
          const match = stdout.toString().match(/LANG=([^\n]+)/);
