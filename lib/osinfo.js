...
function getFQDN() {
  ...
  if (_linux || _darwin) {
    try {
      const stdout = execSync('hostnamectl --json short 2>/dev/null', { stdio: ['pipe', 'pipe', 'ignore'] });
      const json = JSON.parse(stdout.toString());

      fqdn = json['StaticHostname'];
    } catch (e) {
      try {
        const stdout = execSync('hostname -f 2>/dev/null', { stdio: ['pipe', 'pipe', 'ignore'] });
  ...
  if (_freebsd || _openbsd || _netbsd) {
    try {
      const stdout = execSync('hostname 2>/dev/null', { stdio: ['pipe', 'pipe', 'ignore'] });
      fqdn = stdout.toString().split(os.EOL)[0];
    } catch (e) {
      util.noop();
    }
