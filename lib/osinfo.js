      ...
      try {
        ...
        exec('pip -V 2>&1', function (error, stdout) {
          if (!error) {
            ...
            result.pip = pip.toLowerCase().replace(/pip\s+([0-9.]+)\s+from.+/, '$1').trim();
          }
          functionProcessed();
        });
        exec('python3 -V 2>&1', function (error, stdout) {
          if (!error) {
            const python3 = stdout.toString().split('\n')[0] || '';
            result.python3 = python3.toLowerCase().replace('python', '').trim();
          }
          functionProcessed();
        });
        exec('pip3 -V 2>&1', function (error, stdout) {
          if (!error) {
            const pip3 = stdout.toString().split('\n')[0] || '';
            result.pip3 = pip3.toLowerCase().replace(/pip\s+([0-9.]+)\s+from.+/, '$1').trim();
          }
          functionProcessed();
        });
        ...
        exec('java -version 2>&1', function (error, stdout) {
          if (!error) {
            ...
            result.java = java.toLowerCase().replace(/java version\s+"([a-z0-9._-]+)".*/i, '$1').trim();
          }
          functionProcessed();
        });
      } catch (e) {
