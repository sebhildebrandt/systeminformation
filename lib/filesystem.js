        ...
        try {
          ...
          util.promiseAll(workload).then((data) => {
            ...
            devices.forEach((device) => {
              ...
              // SerialNumber value from Win32_DiskDrive is corrupted
              if (serialNum.startsWith('\x03')) {
                const instanceId = util.getValue(lines, 'PNPDeviceId', ':').trim().split('\\')[2];
                if (instanceId) {
                  const instanceIdParts = instanceId.split('&');
                  // instanceId can contain device serial number or be generated by PnP manager
                  // in first case it will be <serial number>[&<some option component>]
                  // in second case it will be opaque generated value that looks like this: 5&105C2DBE&0&000000
                  // https://learn.microsoft.com/en-us/windows-hardware/drivers/install/instance-ids
                  if (instanceIdParts.length <= 2) {
                    serialNum = instanceIdParts[0]
                  }
                }
              }
              if (size) {
