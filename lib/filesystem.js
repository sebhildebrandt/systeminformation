...
function diskLayout(callback) {
  ...
  return new Promise((resolve, reject) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        exec("lshw -xml -class disk", function (error, stdout) {
          if (!error) {
            let devices = stdout.toString().split('node id="disk"');
            devices.shift();
            devices.forEach(function (device) {
              let lines = device.split('\n');
              const logical = getValue(lines, '<logicalname', '>', true).trim().replace(/\/dev\//g, '').replace('<\/logicalname:', '');
              const mediumType = execSync('cat /sys/block/' + logical + '/queue/rotational').toString().split('\n')[0];
              const size = getValue(lines, '<size', '>', true).trim().replace('<\/size:', '');
              if (size && lines.length > 0 && /class="disk"/.test(lines[0])) {
                result.push({
                  type: (mediumType === '0' ? 'SSD' : (mediumType === '1' ? 'HD' : (device.indexOf('SSD') > -1 ? 'SSD' : 'HD'))), // to be tested ... /sys/block/sda/queue/rotational
                  name: getValue(lines, '<product', '>', true).replace('<\/product:', '').trim(),
                  vendor: getValue(lines, '<vendor', '>', true).replace('<\/product:', '').trim(),
                  size: parseInt(size),
                  bytesPerSector: -1,
                  totalCylinders: -1,
                  totalHeads: -1,
                  ...
                  tracksPerCylinder: -1,
                  sectorsPerTrack: -1,
                  firmwareRevision: getValue(lines, '<version', '>', true).replace('<\/version:', '').trim(),
                  serialNum: getValue(lines, '<serial', '>', true).replace('<\/serial:', '').trim(),
                  interfaceType: '',
                })
              }
            });
