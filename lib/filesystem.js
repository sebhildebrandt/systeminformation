      ...
      if (_linux || _freebsd || _openbsd || _netbsd || _darwin) {
        ...
        if (_linux) {
          try {
            ...
            execSync('cat /proc/mounts 2>/dev/null', { stdio: ['pipe', 'pipe', 'ignore'] }).toString().split('\n').filter(line => {
              return line.startsWith('/');
            }).forEach((line) => {
          ...
          if (!error) {
            try {
              ...
              try {
                ...
                try {
                  const out2 = execSync('export LC_ALL=C; lsblk -bPo NAME,TYPE,SIZE,FSTYPE,MOUNTPOINT,UUID,ROTA,RO,RM,LABEL,MODEL,OWNER,GROUP 2>/dev/null; unset LC_ALL', { stdio: ['pipe', 'pipe', 'ignore'] }).toString();
              ...
              devices.forEach((device) => {
                ...
                try {
                  mediumType = execSync('cat /sys/block/' + logical + '/queue/rotational 2>/dev/null', { stdio: ['pipe', 'pipe', 'ignore'] }).toString().split('\n')[0];
                } catch (e) {
                  util.noop();
                }
