// Shared key -> si.function() map, used by si.cjs (node/dist) and si.ts (bun/deno/src).
function run(si, f) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      f = f.replace(/'/g, '');
      if (f === 'a') { si.audio().then(data => { if (data !== null) { resolve({ data, title: 'Audio' }); } else { resolve('not_supported'); } }); }
      else if (f === 'A') { si.displays().then(data => { if (data !== null) { resolve({ data, title: 'Displays' }); } else { resolve('not_supported'); } }); }
      ...
      else if (f === 'f') { si.fsSize().then(data => { if (data !== null) { resolve({ data, title: 'File Size' }); } else { resolve('not_supported'); } }); }
      else if (f === 'F') { si.fsStats().then(data => { if (data !== null) { resolve({ data, title: 'FS Stats' }); } else { resolve('not_supported'); } }); }
      else if (f === 'g') { si.gpu().then(data => { if (data !== null) { resolve({ data, title: 'GPU' }); } else { resolve('not_supported'); } }); }
      else if (f === 'G') { si.pci().then(data => { if (data !== null) { resolve({ data, title: 'PCI' }); } else { resolve('not_supported'); } }); }
      ...
      else if (f === 'n') { si.npu().then(data => { if (data !== null) { resolve({ data, title: 'NPU' }); } else { resolve('not_supported'); } }); }
      else if (f === 'N') { si.npm().then(data => { if (data !== null) { resolve({ data, title: 'NPM Packages' }); } else { resolve('not_supported'); } }); }
      ...
      else if (f === '-') { si.version().then(data => { resolve({ data, title: 'Version' }); }); }
